...
private getMaxRangePercent(interval: string): number {
  switch (interval) {
    ...
    case '1h': return 0.035;     // 3.5%
    case '2h': return 0.08;      // 8%
    case '4h': return 0.12;      // 12%
    case '1d': return 0.15;      // 15%
    default:   return 0.035;     // fallback
  }
}
...
// candles[0] = oldest of the last 45, candles[44] = newest
    ...
    // Loop from oldest possible pattern start → newest
    for (let i = 4; i < candles.length; i++) {  // i = confirmation candle index
      const c1 = candles[i-3];  // oldest in pattern
      const c2 = candles[i-2];
      const c3 = candles[i-1];
      const c4 = candles[i];    // newest / confirmation
if (i-3 < 5) continue;
  ...
  const postStart = i + 1;
        if (postStart < candles.length) {
          ...
          if (wantBullish) {
            ...
            if (minPostLow < patternLow) continue;
          } else {
            const maxPostHigh = Math.max(...postCandles.map(c => c.high));
            if (maxPostHigh > patternHigh) continue;
          }
        }
     //  console.log('PATTERN:', patternName, '|', symbol, '|', wantBullish ? 'BUY' : 'SELL', '| TF:', interval, '| Formed on candles indices:', i-3, 'to', i);
     //   console.log('Candle 1 (oldest):', { time: c1.startTime, open: c1.open, high: c1.high, low: c1.low, close: c1.close });
     //   console.log('Candle 2:', { time: c2.startTime, open: c2.open, high: c2.high, low: c2.low, close: c2.close });
     //   console.log('Candle 3:', { time: c3.startTime, open: c3.open, high: c3.high, low: c3.low, close: c3.close });
      //  console.log('Candle 4 (newest):', { time: c4.startTime, open: c4.open, high: c4.high, low: c4.low, close: c4.close });
        return true;
      }
      // Inside Bar patterns with variable numInside
      for (let numInside = 5; numInside >= 1; numInside--) {
        const motherIndex = i - (numInside + 1);
        if (motherIndex < 0) continue;
             if (motherIndex < 5) continue; 
        ...
        console.log('PATTERN:', patternName, '|', symbol, '|', wantBullish ? 'BUY' : 'SELL', '| TF:', interval, '| Formed on candles indices:', motherIndex, 'to', i);
          console.log('Mother Candle:', { time: mother.startTime, open: mother.open, high: mother.high, low: mother.low, close: mother.close });
          for (let j = 1; j <= numInside; j++) {
            const insideC = candles[motherIndex + j];
          //  console.log(`Inside Candle ${j}:`, { time: insideC.startTime, open: insideC.open, high: insideC.high, low: insideC.low, close: insideC.close });
          }
        //  console.log('Breakout Candle:', { time: breakout.startTime, open: breakout.open, high: breakout.high, low: breakout.low, close: breakout.close });
        //  console.log('Confirmation Candle:', { time: confirm.startTime, open: confirm.open, high: confirm.high, low: confirm.low, close: confirm.close });
          return true;
        }
      }
    }
